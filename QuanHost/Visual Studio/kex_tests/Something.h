...
int encryptMsg(unsigned char key[188], string &msg)
{
    ...
    // Устанавливаем ключ клиента, которому отправляем сообщение
    vector<unsigned char> v_key(188);
    for (int i(0); i < 188; ++i)
        v_key[i] = key[i];
...
int decryptMsg(unsigned char key[188], string& msg)
{
    ...
    // Устанавливаем ключ клиента, от которого получено сообщение
    vector<unsigned char> v_key(188);
    for (int i(0); i < 188; ++i)
        v_key[i] = key[i];
