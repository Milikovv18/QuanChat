...
void sendFile(SOCKET sock, string msg)
{
	if (msg.size() < 4)
	{
		print("[ERRORR] Empty parameter", FOREGROUND_RED);
		busy = false;
		return;
	}
	...
	FILE* in = fopen(msg.substr(3, msg.size()).c_str(), "rb");

	if (in)
	{
		struct _stat64 buf;
		_stat64(msg.substr(3, msg.size()).c_str(), &buf) != 0;
		...
		print("[NOTICE] Sending a " + to_string(buf.st_size) + " byte file \"" + msg.substr(msg.find_last_of("/\\") + 1) + ".quan\"                                   ", 14);
		...
		print("[NOTICE] File \"" + msg.substr(msg.find_last_of("/\\") + 1) + ".quan\" delivered in " + to_string((std::clock() - start) / (double)CLOCKS_PER_SEC) + "s", 14);
	}
	else
		print("[ERRORR] File \"" + msg.substr(3, msg.size()) + "\" not found or its already busy", FOREGROUND_RED);
...
bool connectTor(string address, string name)
{
	...
	while (true)
	{
		...
		if (msg.substr(0, 2) == "/f")
		{
			if (busy)
			{
				print("[ERRORR] File thread is busy!", FOREGROUND_RED);
				continue;
			}
...
bool connectIP(string address, string name)
{
	...
	// И отсылаем его
	iResult = send(ConnectSocket, greeting.c_str(), 1 + name.size(), 0);
	...
	while (true)
	{
		...
		if (msg.substr(0, 2) == "/f")
		{
			if (busy)
			{
				print("[ERROR] File thread is busy!", FOREGROUND_RED);
				continue;
			}
