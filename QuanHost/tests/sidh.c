...
void generateKey(unsigned char PublicKeyB[SIDH_PUBLICKEYBYTES])
{
    //random_mod_order_A(PrivateKeyA);
    random_mod_order_B(PrivateKeyB);

    //EphemeralKeyGeneration_A(PrivateKey, PublicKey);                           // Get some value as Alice's secret key and compute Alice's public key
    EphemeralKeyGeneration_B(PrivateKeyB, PublicKeyB);                           // Get some value as Bob's secret key and compute Bob's public key
}
void agreeKeys(unsigned char SharedSecret[SIDH_BYTES], unsigned char PublicKey[SIDH_PUBLICKEYBYTES])
{
    EphemeralSecretAgreement_B(PrivateKeyB, PublicKey, SharedSecret);            // Alice computes her shared secret using Bob's public key
}