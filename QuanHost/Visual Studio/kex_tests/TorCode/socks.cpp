...
namespace socks5cpp {
	...
	SocksClient::SocksUrl SocksClient::scanURL(const std::string _url)
	{
		...
		const char * url_t = _url.c_str();
		char ip_t[256];
		unsigned int port;
		// scan string
		if (sscanf(url_t, "%100[^:]:%d", ip_t, &port))
		{
			res.ip = ip_t;
			res.port = port;
		...
		sscanf(res.ip.c_str(), "%hhu.%hhu.%hhu.%hhu", &res.ipv4[0], &res.ipv4[1],
	...
	int SocksClient::connect(SOCKET& sfd)
	{
		...
		::send(sfd, reinterpret_cast<const char*>(buffer.data()), int(buffer.size()), 0);
		...
		buffer.push_back(uint8_t(targetUrl.ip.length())); // connecting to domain name
	...
	{
		return ::send(sfd, _Buffer, int(_Size), 0);
	}

	int SocksClient::recvPacket(SOCKET& sfd, char * _Buffer, const size_t _Size)
	{
		return ::recv(sfd, _Buffer, int(_Size), 0);
	}
